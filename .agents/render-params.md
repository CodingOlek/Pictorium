...
| `hideLogo` | mai dal client (solo banner Stremio) | solo query `hideLogo=1`: salta il composite logo film (fetch tenuto per i colori accent). Veicolo del banner Nuvio (`forceShape` + `hideLogo` in `catalog-handler.ts`): Nuvio sovrappone già il logo da catalogo. Col banner il badge genere va in basso a DESTRA (ancoraggio destro, niente shift landscape) e il network segue i rami "senza logo" (top-left / a fianco del nastro) |
