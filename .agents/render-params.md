...
| Offset badge superiore (`topBadgeOffsetX/Y`) | Solo stili centrati: `left = center + tox`, `top = 0 + toy` (px, default 0) + `pillTopGap` per la pill (gap fisso `round(18 * CH / 570)`, indipendente dal badge qualità); il nastro resta ancorato; la matematica overlap usa `finalRankTop + h` |
