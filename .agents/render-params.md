...
| Sfondo pill/bar | bar (`buildGenreBarSvg`) = gradiente satinato `satinPillStops(bottomLight)` full-width + bordo profilo 1.5px adattivo (metà esterna tagliata dal viewport sui bordi full-bleed, sub-visibile); pill genere = gradiente satinato `satinPillStops(bottomLight)` (polarità del fondo, non del top); pill ranking/extra = `satinPillStops(topLight)` |
...
| Ombra | `default`/extra-default/`pill` ranking-extra: ombra 3D singola stile nastro (`dx=3, dy=3, blur 3.5, 0.65`, canvas = box esatta, coda tagliata come il nastro); `badgeShadowBox` resta per stime overflow — `blur = max(round(boxH * 0.20), 4)`, `off = max(round(boxH * 0.10), 2)` dove ancora usato |
| Sfondo | `default`/extra-default/`pill` — gradiente satinato `satinPillStops(topLight)` + bordo sagomato polarizzato 1.5px (`topLight ? "rgba(0,0,0,0.12)" : "rgba(255,255,255,0.22)"`) + ombra 3D singola, canvas = box esatta (`totalW × boxH`); `netflix` — nastro invariato; `colored` — tinta accent piatta + `textColorForBg` (lo stile `bar` del ranking è rimosso) |
...
| Offset badge superiore (`topBadgeOffsetX/Y`) | Solo stili centrati: `left = center + tox`, `top = 0 + toy` (px, default 0); il nastro resta ancorato; la matematica overlap usa `finalRankTop + h` |
