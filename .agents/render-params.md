...
| Ombra | `default`/extra-default: **nessuna ombra esterna** (finitura quality-badge; `badgeShadowBox` resta per stime overflow) — `blur = max(round(boxH * 0.20), 4)`, `off = max(round(boxH * 0.10), 2)` dove ancora usato |
| Sfondo | `default`/extra-default — gradiente satinato `satinPillStops(topLight)` + bordo sagomato polarizzato 1.5px (`topLight ? "rgba(0,0,0,0.12)" : "rgba(255,255,255,0.22)"`), canvas = box esatta (`totalW × boxH`, zero padding ombra); `netflix` — nastro invariato; `pill` — satinato come default + stroke 1px (testo adattivo invariato); `colored` — tinta accent piatta + `textColorForBg` (lo stile `bar` del ranking è rimosso) |
