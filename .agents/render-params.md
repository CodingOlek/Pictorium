...
| Ombra | `default`/extra-default/`pill` ranking-extra: nessuna ombra esterna (finitura pulita; `badgeShadowBox` resta per stime overflow) — `blur = max(round(boxH * 0.20), 4)`, `off = max(round(boxH * 0.10), 2)` dove ancora usato |
| Sfondo | `default`/extra-default/`pill` — gradiente satinato `satinPillStops(topLight)` + bordo sagomato polarizzato 1.5px (`topLight ? "rgba(0,0,0,0.12)" : "rgba(255,255,255,0.22)"`), nessuna ombra esterna, canvas = box esatta (`totalW × boxH`); `netflix` — nastro invariato (con ombra 3D propria); `colored` — tinta accent piatta + `textColorForBg` (lo stile `bar` del ranking è rimosso) |
