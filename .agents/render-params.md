...
| `gradHeight` | `gradientHeight` | `qGradHeight` — alimenta l'altezza del gradiente/sfocatura (blurHeight; default 30 portrait, 20 landscape solo da query/config assenti; mapping non-clean senza valore congelato: 20). Post-selezione solo Stremio unmapped: se il poster finale ha testo incorporato, i default iniettati nell'URL non vincono sul profilo non-clean (20/80, come il client) |
