...
| Offset badge superiore (`topBadgeOffsetX/Y`) | Solo stili centrati: `left = center + tox`, `top = 0 + toy` (px, default 0) + `pillTopGap` per la pill (fondo pill allineato al fondo badge qualità: `netBaseTop + qualityH - pillH`, fallback al gap standard senza qualità); il nastro resta ancorato; la matematica overlap usa `finalRankTop + h` |
